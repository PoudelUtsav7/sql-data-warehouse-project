-- Here doing Data Transformation and Data Cleansing

USE DataWarehouse;
GO

SELECT cust_id, COUNT(*) FROM bronze.crm_cust_info
GROUP BY cust_id
HAVING COUNT(*) > 1 OR cust_id IS NULL;


-- REMOVING THE VALUE WHICH ARE REPEATED IN THE PRIMARY KEY.
...
-- IN OUR DATAWAREHOUSE, WE AIM TO STORE CLEAR AND MEANINGFUL VALUES RATHER THAN 
-- USING ABBREVIATED TERMS: EXAMPLE: F -> Female, M -> Male
...
--
-- NOW WE WILL INSERT IT INTO TABLE
...
-- INSERTING INTO TABLE BY FIXING THE DATE THROUGH LEAD/ LAG FUNCTION
...
-- Checking for Invalid Dates (Because we need to cast it into DATES but 
...
-- There are 0 so replacing those with NULL with NULLIF()
...
SELECT 
	...
	CASE WHEN sls_order_dt = 0  OR LEN(sls_order_dt) != 8 THEN NULL
		 ELSE CAST(CAST(sls_order_dt AS VARCHAR) AS DATE) -- We cant cast integer to date we need t oconvert fist into varchar
	END AS sls_order_dt,
...
-- 
--= Checking another condition because order date must always be earlier than the shipping date or due date


-- Checking for the invalid dates 
SELECT * FROM bronze.crm_sales_details
WHERE sls_order_dt > sls_ship_dt OR sls_order_dt > sls_due_dt; -- No transformation needed
...
-- Now we found out there is issue so we will fix it and here are some Rules to fix it
-- RULES 
...
-- For gen (DATA STANDARDIZATION & CONSISTENCY)
